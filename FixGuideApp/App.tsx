...
export default function App() {
    ...
    try {
      ...
      await lm.download({
        onProgress: (p) => {
          ...
          setModelStatus(`Downloading Gemma 270M... ${Math.round(p * 100)}%`);
        },
      });
      lmRef.current = lm;
      setModelReady(true);
      ...
      setModelStatus('☁️  Cloud mode · Gemini 2.0 Flash');
    }
    setState('idle');
  }
  ...
  function listenOnce(): Promise<string> {
    return new Promise(resolve => {
      ...
      Voice.onSpeechEnd = () => setTimeout(() => finish(''), 500);
      Voice.start('en-US').catch(() => finish(''));
      setTimeout(() => { Voice.stop().catch(() => {}); }, 9000);
    });
  }
  ...
  function speakAndWait(text: string): Promise<void> {
    return new Promise(resolve => {
      ...
      Tts.speak(text);
      setTimeout(resolve, text.length * 70 + 1500);
    });
  }
  ...
  async function startAgent() {
    ...
    setState('speaking');
    await speakAndWait("Hey, I'm here. Tell me what's going on and I'll help you fix it.");
    const msgs: Message[] = [];
    while (runningRef.current) {
      ...
      try {
        ...
        setVerdict(v);
        setResponse(reply);
        const updated: Message[] = [...nextMsgs, { role: 'assistant', content: reply }];
        msgs.push(...updated.slice(msgs.length));
        setHistory([...updated]);
        setState('speaking');
        await speakAndWait(reply);
      } catch {
        await speakAndWait('Connection error, please try again.');
        break;
      }
    }
    setState('idle');
  ...
  const sl = { idle: 'Tap Start', downloading: `Downloading ${downloadPct}%`, listening: '🎤  Listening...', thinking: '🧠  Thinking...', speaking: '🔊  Speaking...' }[state];
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView contentContainerStyle={s.scroll}>
        ...
        <View style={s.statePill}>
          {(state === 'thinking' || state === 'downloading') && <ActivityIndicator size="small" color="#f59e0b" style={{ marginRight: 8 }} />}
          <Text style={[s.stateText, { color: state === 'listening' ? '#00d4aa' : state === 'thinking' ? '#f59e0b' : state === 'speaking' ? '#a78bfa' : state === 'downloading' ? '#f59e0b' : '#64748b' }]}>{sl}</Text>
        </View>
        ...
        <View style={s.controls}>
          {state === 'idle'
            ? <TouchableOpacity style={s.startBtn} onPress={startAgent}><Text style={s.startTxt}>▶  Start Agent</Text></TouchableOpacity>
            : state !== 'downloading'
              ? <TouchableOpacity style={s.stopBtn} onPress={stopAgent}><Text style={s.stopTxt}>⏹  Stop</Text></TouchableOpacity>
              : null
          }
        </View>
...
const s = StyleSheet.create({
  ...
  motto: { color: '#00d4aa', fontSize: 24, fontWeight: '900', letterSpacing: -0.5, marginBottom: 32 },
  ...
  startBtn: { backgroundColor: '#00d4aa', paddingHorizontal: 48, paddingVertical: 16, borderRadius: 100 },
  startTxt: { color: '#040a0f', fontSize: 16, fontWeight: '800' },
